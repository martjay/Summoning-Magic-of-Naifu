...
### 前言
NovelAI发布至今已有数月之久，各路圈内圈外人士都来凑热闹。（编程圈，AI圈，绘画圈......）
...
1. 一些基础概念：
   - Stable Diffusion（简称SD）是一个基于Latent Diffusion Models（潜在扩散模型，LDMs）的文图生成（text-to-image）模型，开源免费。
   - Web-UI是SD的原生前端实现。
   - NovelAI 就是用Danbooru的图片在Stable Diffusion的基础上做了模型的优化训练（fine-tune）。
   - Naifu是NovelAI的官方前端。
   - 现在大部分本地部署的Naifu使用的模型基本都是NovelAI泄露出的两个模型。

2. Naifu是官方版本，性能与质量保证本身就是其他框架所不能比的，但新版的Web-UI（不同模型）也能跟官方版媲美。
3. 界面简洁舒适，没有复杂的参数设置。
4. 语法没有Web-UI那么复杂，同样对新手友好。但更复杂的语法代表了更多的可能性与更高的灵活度，所以在体验完Naifu后不妨再去尝试Web-UI的超位魔法。
5. 现在关于Web-UI的教程数不胜数，而Naifu的教程却寥寥无几，所以想分享自己的经验。
...
你感到疑惑，却又被那曼妙的身姿所吸引，你不住地点击图片。突然，一阵耀眼的金光从屏幕中闪现。『啊！』
恍然间，你失去了知觉。
